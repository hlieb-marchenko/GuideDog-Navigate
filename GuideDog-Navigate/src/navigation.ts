...
export async function fetchCrossingInfo(
  ...
  if (crossingCache.has(key)) {
    return crossingCache.get(key)!;
  }
  ...
  try {
    ...
    const hasTrafficLight = elements.some(
      (e: any) =>
        e.tags?.highway === "traffic_signals"
    );

    const crossings = elements.filter(
      (e: any) =>
    ...
    const result: CrossingInfo = {
      ...
      isSignalised: crossings.some(
        (e: any) =>
          e.tags?.crossing === "traffic_signals"
      ),
      hasTrafficLight,
    };
...
export async function fetchStreetName(
  ...
  try {
    ...
    const named = (data.elements ?? []).find(
      (e: any) => e.tags?.name
    );
...
export async function buildVoiceInstruction(
  ctx: VoiceContext
): Promise<string> {
  const steps = metersToSteps(
    ctx.distanceM,
    ctx.stepLength
  );
  ...
  if (ctx.isStart && ctx.startBearing !== null) {
    text += `Start heading ${bearingToCardinal(
      ctx.startBearing
    )}. `;
  }
  ...
  if (ctx.crossing.hasCrossing) {
    if (
      ctx.crossing.hasTrafficLight ||
      ctx.crossing.isSignalised
    ) {
      text +=
        " Signalised crossing ahead. Wait for the green signal.";
    } else {
      text +=
        " Crossing ahead. Listen carefully for traffic.";
    }
  }
